...
export class DataServiceService {
  ...
  private globalDataUrl = `https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/07-12-2020.csv`;
      ...
      map(result=>{
        ...
        rows.forEach(row=>{
          ...
          cols.forEach((value, index)=>{
            let dwd : DateWiseData ={
              cases : +value,
              country: country,
              date : new Date(Date.parse(date[index]))
            }
            mainData[country].push(dwd)
          })
